...
4. Uses Chrome headless to print to PDF
...
def build_clean_html(original_html, note_title=None):
    ...
    If note_title is provided and the body doesn't already contain an <h1>, prepend it."""
    body_html = original_html
    if note_title:
        has_h1 = bool(re.search(r'<h1\b', body_html, re.IGNORECASE))
        if not has_h1:
            body_html = f'<h1 class="note-title">{note_title}</h1>\n{body_html}'
...
body {{
    ...
    line-height: 1.85; color: #2a2a2a; max-width: 650px;
    margin: 0 auto; padding: 28px 24px; font-size: 15px; background: #fff;
}}
.anchor {{ height: 1px; width: 1px; color: #fff; font-size: 1px; }}
.note-title {{ font-size: 22px; line-height: 1.5; margin-bottom: 16px; color: #111; text-align: center; font-weight: 700; }}
h1 {{ font-size: 22px; line-height: 1.4; margin-bottom: 6px; color: #111; }}
.meta {{ color: #999; font-size: 13px; margin-bottom: 2px; }}
.tags {{ color: #576b95; font-size: 13px; margin-bottom: 14px; }}
blockquote {{ border-left: 3px solid #b4c0cc; background: #f8f9fa; padding: 8px 16px; margin: 12px 0; color: #555; font-size: 14px; }}
h2 {{ font-size: 18px; margin: 22px 0 8px; color: #111; }}
p {{ margin: 7px 0; }}
strong {{ color: #111; }}
a {{ color: #576b95; word-break: break-all; }}
img {{ max-width: 100%; height: auto; display: block; margin: 12px auto; border-radius: 4px; }}
.footer {{ margin-top: 22px; padding-top: 14px; border-top: 1px solid #eee; color: #888; font-size: 13px; }}
...
def chrome_print_to_pdf(html_path, pdf_path):
    ...
    # Check if PDF was created (ignore stderr encoding issues)
